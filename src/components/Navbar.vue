...
<template>
  <div class="nav-container bg-black">
    ...
    <div
      class="main-nav bg-black d-flex justify-space-between align-center"
    ...
    >
      ...
      <div class="logo-section">
        <div class="p-4">
          <router-link to="/">
            <img :src="logoImage" alt="Kotype Logo" class="logo-image" width="100px" height="auto"/>
          </router-link>
        </div>
      </div>

      <!-- Desktop Menu -->
      <div v-if="!isMobile" class="d-flex justify-center align-center flex-grow-1 h-100">
        <!-- Menu Items -->
        <div
          v-for="item in menuItems"
          :key="item.title"
          class="menu-item-col px-4 px-md-8"
      ...
      <div v-if="isMobile" class="hamburger-section bg-black d-flex justify-end align-center">
        <div class="p-3 p-md-4 d-flex justify-center align-center">
          <v-btn icon @click="drawer = !drawer" class="hamburger-btn">
            <v-icon :size="isMobile ? 'large' : 'x-large'">mdi-menu</v-icon>
          </v-btn>
        </div>
      </div>
    </div>
      ...
      <v-list color="black">
        ...
        >
          ...
          <template v-slot:append v-if="item.count">
            <span class="text-white count-badge">{{ item.count }}</span>
          </template>
        </v-list-item>
